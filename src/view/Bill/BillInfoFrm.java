...
public class BillInfoFrm extends javax.swing.JFrame implements ActionListener{
    ...
    private Bill bill;
    private JLabel lbId, lbRoomName, lbClient, lbMonth, lbRFee, lbSSFee, lbTotal;
    ...
    public BillInfoFrm(User user, Bill bill){
        ...
        lbRoomName = new JLabel(bill.getContract().getRooms().get(0).getRoom().getName() + "");
...
//        scrollPane2.setPreferredSize(new Dimension(scrollPane2.getPreferredSize().width, 300));
        ...
        float total = bill.getRentingFee();
        
        String[] columnNames = {"Id", "Tên dịch vụ", "Số tháng trước", "Số tháng này", "Đơn giá", "Thành tiền"};
        String[][] value = new String[2][6];
        for(int i=0; i< 2; i++){
            RentedRoomMonthlyService sv = bill.getContract().getRooms().get(0).getRrmService().get(i);
            value[i][0] = sv.getId() +"";
        ...
        ArrayList<RentedRoomStaticService> rrss = bill.getContract().getRooms().get(0).getRrsService();
        String[][] value2 = new String[rrss.size()][6];
        for(int i= 0; i< rrss.size(); i++){
            RentedRoomStaticService sv = rrss.get(i);
            value2[i][0] = sv.getId() +"";
